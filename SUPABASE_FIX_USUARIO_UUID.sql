...
alter table public."FECHAMENTOS_CAIXA" add column if not exists fechado_por_id uuid;
alter table public."FECHAMENTOS_CAIXA" add column if not exists reaberto_por_id uuid;
alter table public."MOVIMENTACOES_ESTOQUE" add column if not exists usuario_id uuid;
alter table public."MOVIMENTACOES_CAIXA" add column if not exists usuario_id uuid;
alter table public."DESPESAS" add column if not exists usuario_id uuid;

alter table public."VENDAS" alter column usuario_id set default auth.uid();
alter table public."MOVIMENTACOES_ESTOQUE" alter column usuario_id set default auth.uid();
alter table public."MOVIMENTACOES_CAIXA" alter column usuario_id set default auth.uid();
alter table public."DESPESAS" alter column usuario_id set default auth.uid();
...
alter table public."ALTERACOES_VENDA" add column if not exists alterado_por_id uuid;
...
alter table public."CAIXA_TROCO" add column if not exists usuario_id uuid;

insert into public."CAIXA_TROCO" (id, saldo_atual)
values (1, 0)
...
alter table public."MOVIMENTACOES_TROCO" add column if not exists usuario_id uuid;
...
begin
  ...
  loop
    ...
    values (
      ...
      -rec.quantidade, 'venda', v_sale.id, auth.uid()
    );
  end loop;
...
begin
  ...
  loop
    ...
    values (
      ...
      'cancelamento', p_venda_id, auth.uid()
    );
  end loop;
...
begin
  ...
  loop
    if rec.diferenca <> 0 then
      ...
      values (
        ...
        -rec.diferenca, 'edicao venda', p_venda_id, auth.uid()
      );
    end if;
  end loop;
  ...
  v_alterado_por_text = coalesce(v_alterado_por::text, '');

  insert into public."ALTERACOES_VENDA" (
    venda_id, dados_anteriores, dados_novos, alterado_por, alterado_por_id, motivo
  )
  values (
    p_venda_id,
