-- Fumacinha - correcao complementar do Controle da Loja
-- Execute no SQL Editor do Supabase da Fumacinha.
-- Idempotente: cria/ajusta tabelas sem apagar dados existentes.
...
begin
  if not exists (
    ...
    where c.contype = 'f'
      and c.conrelid = 'public."MOVIMENTACOES_ESTOQUE"'::regclass
      and c.confrelid = 'public."PRODUTOS"'::regclass
      and a.attname = 'produto_id'
  ) and exists (
    select 1
    from pg_attribute a
    where a.attrelid = 'public."MOVIMENTACOES_ESTOQUE"'::regclass
      and a.attname = 'produto_id'
  ...
  if not exists (
    ...
    join pg_attribute a on a.attrelid = c.conrelid and a.attnum = any(c.conkey)
    where c.contype = 'f'
      and c.conrelid = 'public."ITENS_VENDA"'::regclass
      and c.confrelid = 'public."VENDAS"'::regclass
      and a.attname = 'venda_id'
  ) then
